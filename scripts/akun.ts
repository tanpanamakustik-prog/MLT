...
const lama = db.prepare('SELECT * FROM pengguna WHERE username = ?').get(username) as any;
...
if (ada('nonaktif') || ada('aktifkan')) {
  ...
  const aktif = ada('aktifkan') ? 1 : 0;
  db.prepare('UPDATE pengguna SET aktif = ? WHERE id = ?').run(aktif, lama.id);
...
for (const [label, tabel, nilai] of [['karyawan', 'karyawan', karyawanId], ['customer', 'customer', customerId]] as const) {
  if (nilai && !db.prepare(`SELECT id FROM ${tabel} WHERE id = ?`).get(Number(nilai))) {
...
if (lama) {
  const sandiBaru = sandiDiberikan ? hashSandi(sandiDiberikan) : lama.kata_sandi;
  db.prepare(
    `UPDATE pengguna SET nama = ?, peran = ?, kata_sandi = ?, karyawan_id = ?, customer_id = ? WHERE id = ?`
  ).run(
  ...
  db.prepare(
    'INSERT INTO pengguna (username, nama, kata_sandi, peran, karyawan_id, customer_id) VALUES (?, ?, ?, ?, ?, ?)'
  ).run(
    username,
    argumen('nama') ?? username,
    hashSandi(sandi),
    peran,
